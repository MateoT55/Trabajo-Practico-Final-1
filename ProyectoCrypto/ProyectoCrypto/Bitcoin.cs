...
{
    // 2. Herencia: Bitcoin hereda de MonedaBase
    public class Bitcoin : MonedaBase
    {
        // 6. Enum (usado para el símbolo de la API)
        public enum ApiSymbol { BTCUSDT }

        public Bitcoin() : base("Bitcoin", "BTC") { }

        // 8. Conexión a una API externa (Binance)
        public override async Task<double> ObtenerPrecioUSD()
        {
            string urlApi = $"https://api.binance.com/api/v3/ticker/price?symbol={ApiSymbol.BTCUSDT}";
